...
France.flags = {}
France.flags.toggles = {}
France.flags.sliders = {}
France.functions = {}
France.currentTab = nil
...
function France.functions:Drag(frame,hold)
...
function France.functions:ChangeSection(NewSection)
	if France.currentTab == NewSection then return end
	if France.currentTab == nil then
		France.currentTab = NewSection
		France.currentTab.Visible = true
		return		
	end
	France.currentTab.Visible = false
	France.currentTab = NewSection
...
function France:CreateWindow(WindowText)
	...
	France.functions:Drag(Main, UiTitle)
	local Sections = {}
	function Sections:CreateSection(SectionName)
		...
		if France.currentTab == nil then
			France.functions:ChangeSection(TabMain)
		...
		function ObjectTbl:CreateToggle(toggleTxt, flag, func)
			France.flags.toggles[flag] = false
			...
			local function togglefunc()
				France.flags.toggles[flag] = not France.flags.toggles[flag]
				if isFunctionToggle then func(France.flags.toggles[flag]) end
				France.functions:Tween(ToggleStatus, {Time = 0.1, Style = "Linear", Direction = "InOut"}, {ImageColor3 = France.flags.toggles[flag] and Color3.fromRGB(0, 255, 50) or Color3.fromRGB(255, 20, 24)})
			end
		...
		function ObjectTbl:CreateSlider(SliderTxt, flag, min, max, precise, func)
            ...
            Slider.MouseButton1Down:Connect(function()
                ...
                Connection = RunService.Heartbeat:Connect(function()
                    ...
                    France.functions:Tween(sliderfiller, "Default", {Size = UDim2.new(percent, 0, 1, 0)})
                    sliderval.Text = tostring(Value) .. " / " .. max;
                    France.flags.sliders[flag] = Value;
